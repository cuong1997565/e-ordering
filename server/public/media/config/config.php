...
$token = isset($_GET['akey'])?$_GET['akey']:'';

if($token == '')
{
	// Try to get from referer => This is use in case of upload or delete
    $parts = parse_url($_SERVER['HTTP_REFERER']);
    parse_str($parts['query'], $query);
    $token = $query['akey'];
}

$url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . '://' . $_SERVER['SERVER_NAME'] . '/api/users/check?token=' . $token; //

$result = curl($url);
$result =  json_decode($result);

if(isset($result) && $result->valid == 1)
{
	// Allow access
}
else
{
	// die('Invalid user authentication'); // Comment this for dev purpose
	console.log('Dev: Hey, uncomment the above line when deploy');
}
...
$config = array(
	...
	*/
	...
	|
	*/
	'thumbs_base_path' => '../files/media/thumb/',
