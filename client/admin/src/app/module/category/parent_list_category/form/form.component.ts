...
export class FormComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.route.snapshot.params['category']) {
            this.app.get('categories', {'id': this.route.snapshot.params['category']}).subscribe((res: any) => {
                this.sub_category = res.data[0];
                this.fd.setData({parent_id: this.sub_category.id});
            });
        }
    ...
    save() {
        ...
        if (this.route.snapshot.params['id']) {
            this.app.post(this.ulr, this.fd.form.value).subscribe((res: any) => {
                ...
                this.router.navigate(['/parent_list_category', this.sub_category.id]);
            }, (err) => {
